...
double LAMBDA = 1e-4;
double LAMBDAH = (layers > 2) ? 1e-5 : 1e-4;
double DROP;
...
void RNN::save(string fname) {
  ...
  out << Wf << endl << Vf << endl << bhf << endl;
  out << Wb << endl << Vb << endl << bhb << endl;
  for (uint l=0; l<layers; l++) {
    out << WWff[l] << endl << WWfb[l] << endl
        << VVf[l] << endl << bbhf[l] << endl;
    out << WWbb[l] << endl << WWbf[l] << endl
        << VVb[l] << endl << bbhb[l] << endl;
  }
  out << Wfo << endl << Wbo << endl;
  for (uint l=0; l<layers; l++) {
    out << WWfo[l] << endl << WWbo[l] << endl;
  }
  out << Wo << endl << bo << endl;
}
...
RNN::train(vector<vector<string> > &sents,
  ...
  ostringstream strS;
  strS << "models/drnt_weibo_" << layers << "_" << nhf << "_"
        << nhf << "_" << DROP << "_"
  ...
  for (epoch=0; epoch<MAXEPOCH; epoch++) {
    ...
    if (epoch % 5 == 0) {
      ...
      Matrix<double, 3, 2> resVal = testSequential(validX, validL);
      Matrix<double, 3, 2> resTest = testSequential(testX, testL);
      cout << "P, R, F1 (valid):\n" << resVal << endl;
      cout << "P, R, F1 (test):\n" << resTest << endl << endl;
...
Matrix<double, 3, 2> RNN::testSequential(vector<vector<string> > &sents,
  ...
  for (uint i=0; i<sents.size(); i++) {
    ...
    for (uint j=0; j<labels[i].size(); j++) {
      t = labels[i][j];
      y = labelsPredicted[j];
    ...
    for (uint a=0; a<tru.size(); a++) {
      ...
      for (uint b=0; b<pred.size(); b++) {
        ...
        uint lmax = max(truSpan.first, predSpan.first);
        uint rmin = min(truSpan.second, predSpan.second);
        uint overlap = (rmin > lmax) ? (rmin - lmax) : 0;
  ...
  double recallProp = (nExprTrue==0) ? 1 : recallNumerProp/nExprTrue;
  double f1Prop = (precisionProp + recallProp == 0) ? 0 : (2*precisionProp*recallProp)/(precisionProp+recallProp);

  Matrix<double, 3, 2> results;
  results << precisionProp, precisionProp,
             recallProp, recallProp,
             f1Prop, f1Prop;
  return results;
}
...
int main(int argc, char **argv) {
  fold = (argc > 1) ? atoi(argv[1]) : 0;
  srand(135);
  cout << setprecision(6);
  cout << "DRNT for Weibo Comments - Fold " << fold << endl;

  LookupTable LT;
  LT.load("embeddings-original.EMBEDDING_SIZE=25.txt", 268810, 25, false);

  vector<vector<string> > X, T;
  readSentences(X, T, "weibo_drnt_data/weibo_comments.txt");

  cout << "Loaded " << X.size() << " sentences" << endl;

  unordered_map<string, set<uint> > sentenceIds;
  set<string> allDocs;
  ifstream in("weibo_drnt_data/sentenceid.txt");
  string line;
  uint numericId = 0;
  ...
  ifstream in4("weibo_drnt_data/datasplit/doclist.mpqaOriginalSubset");
  while(getline(in4, line))
    allDocs.insert(line);

  ifstream in2("weibo_drnt_data/datasplit/filelist_train"+to_string(fold));
  ...
  ifstream in3("weibo_drnt_data/datasplit/filelist_test"+to_string(fold));
  ...
  cout << "Train: " << trainX.size() << " | Test: " << testX.size() << " | Valid: " << validX.size() << endl;

  Matrix<double, 6, 2> best = Matrix<double, 6, 2>::Zero();
  double bestDrop;
  for (DROP=0; DROP<0.1; DROP+=0.2) {
